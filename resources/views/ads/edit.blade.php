...
>
    ...
    <div class="mt-6 rounded-3xl border border-white/10 bg-white/[.05] p-6 shadow-2xl shadow-black/20 backdrop-blur-xl">
        <form method="POST" action="{{ route('ads.update', $ad) }}" class="space-y-7" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <section class="space-y-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Virsraksts un apraksts būs redzams sludinājuma detalizētajā skatā.
                    </p>
                </div>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($fuelTypes as $key => $label)
                                <option class="bg-slate-950" value="{{ $key }}" @selected(old('fuel_type', $ad->fuel_type) === $key)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($gearboxTypes as $key => $label)
                                <option class="bg-slate-950" value="{{ $key }}" @selected(old('gearbox_type', $ad->gearbox_type) === $key)>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($bodyTypes as $bodyType)
                                <option class="bg-slate-950" value="{{ $bodyType }}" @selected(old('body_type', $ad->body_type) === $bodyType)>
                                    {{ $bodyType }}
                                </option>
                            @endforeach
                        </select>
                ...
                @if($ad->images->count())
                    <div class="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
                        @foreach($ad->images as $image)
                            <div class="aspect-square overflow-hidden rounded-2xl border border-white/10 bg-slate-900">
                                <img
                        ...
                        <div class="mt-4 grid gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    step="0.01"
                                    name="buyout_price"
                                    value="{{ old('buyout_price', $auction->buyout_price) }}"
                                    class="mt-2 w-full rounded-2xl border-white/10 bg-slate-950/70 text-white placeholder:text-slate-500 focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20"
                                >
